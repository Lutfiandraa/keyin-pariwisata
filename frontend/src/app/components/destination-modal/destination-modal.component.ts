      ...
      <div class="bg-white rounded-lg sm:rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] sm:max-h-[85vh] overflow-y-auto animate-pop-in"
        ...
        <div class="relative h-40 sm:h-48 overflow-hidden">
          ...
          <button (click)="closeModal()" 
                  class="absolute top-2 right-2 sm:top-3 sm:right-3 bg-white/90 hover:bg-white text-gray-900 rounded-full p-1.5 sm:p-2 transition-all duration-300 transform hover:scale-110">
        ...
        <div class="p-4 sm:p-5">
          <div class="prose max-w-none">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900 mb-3">About {{ destination.name }}</h3>
            <p class="text-gray-700 leading-relaxed text-sm sm:text-base line-clamp-6">{{ destination.article }}</p>
          </div>
          ...
          <div class="mt-5 sm:mt-6 flex flex-col sm:flex-row gap-3">
            ...
            <button (click)="closeModal()" 
                    class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-900 font-semibold px-4 py-2 sm:px-5 sm:py-2.5 rounded-lg transition-all duration-300 text-sm sm:text-base">
              Close
            </button>
          </div>
        </div>
